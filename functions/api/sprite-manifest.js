...
function expandCharacterPrefixes(char, action, filenamePrefixes){
  const out=[];
  for(const fp of filenamePrefixes){
    // Most of your current public IDs are at Cloudinary root, e.g. ax_idle_001.
    out.push(fp);
    // Also support public IDs that really include the media-library folder tree.
    out.push(`ctwgame/characters/${char}/${action}/${fp}`);
    out.push(`characters/${char}/${action}/${fp}`);
  }
  return out;
}
...
const BACKGROUND_PREFIXES = {
  day: {
    sky: ['day_sky', 'ctwgame/layers/backgrounds/day_sky', 'layers/backgrounds/day_sky'],
    hills: ['day_hills', 'ctwgame/layers/middle-ground/day_hills', 'layers/middle-ground/day_hills'],
    ground: ['day_ground', 'ctwgame/layers/ground/day_ground', 'layers/ground/day_ground']
  },
  night: {
    sky: ['night_sky', 'ctwgame/layers/backgrounds/night_sky', 'layers/backgrounds/night_sky'],
    hills: ['night_hills', 'ctwgame/layers/middle-ground/night_hills', 'layers/middle-ground/night_hills'],
    ground: ['night_ground', 'ctwgame/layers/ground/night_ground', 'layers/ground/night_ground']
  }
};
...
export async function onRequestGet(context){
  ...
  for(const [char,spec] of Object.entries(CHARACTER_SPECS)){
    ...
    for(const [action,prefixes] of Object.entries(spec.actions)){
      const resources=await listMany(ctx, expandCharacterPrefixes(char, action, prefixes));
