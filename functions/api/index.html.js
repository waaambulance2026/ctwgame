...
<title>Ax + Pura Waaambulance Quests</title>
<!-- ============================================================
SIMPLE EDITOR NOTES - SAFE AREAS TO CHANGE
Use Ctrl+F and search EDIT SECTION to jump around this file.

1) EDIT SECTION 17: SPRITE PATHS
   Change character animation image links here only.
   Keep {n} when your files are named 001, 002, 003.
   Example: ax_idle_{n}.png loads ax_idle_001.png, ax_idle_002.png, etc.

2) EDIT SECTION 18A: ANIMATION SPEEDS
   Lower numbers = faster animation. Higher numbers = slower animation.
   This controls idle, walk and jump timing.

3) EDIT SECTION 18B: FRAME COUNTS + CHARACTER SIZE
   This controls forced frame counts, per-character size, and per-action scale.
   Ax now uses: idle 10, walk 8, jump 10.

4) EDIT SECTION 20: LEVEL DATA
   Add or edit buildings, tutorial boards, tokens, hazards, finish flags and level distance.

5) PLACE_IMAGES
   Put direct Cloudinary image links here for buildings, signs, tokens and hazards.
   Links must start with: https://res.cloudinary.com/dpwlfmhia/image/upload/...

Do not edit random code lower down unless you are fixing a specific function.
Search the EDIT SECTION name first.
============================================================ -->

<!-- Level 1 object upgrade: image items, vanish-on-collect, Mario-style stomp hazards, level-building checklist support. -->
<!-- ============================================================
SAFE REVERTED FIX BUILD
This file is based on the version you put back, not the broken cleaned file.
Only safe comments/status text were corrected here; the working game code structure is preserved.
Key search terms:
SPRITE PATHS
ACTION_FRAME_OVERRIDES
CHARACTER_CONFIGS
...
<!-- ============================================================
EDIT SECTION 02: CSS STARTS HERE
This section controls colours, sizes, layers, menus, editor panels and mobile layout.
Use Ctrl+F and search EDIT SECTION to jump around safely.
============================================================ -->
<style>
/* EDIT SECTION 03: MASTER SIZING VARIABLES - easiest values to change safely */
:root{
  --pink:#d282c2; --blue:#76C7D8; --yellow:#F5C542; --ink:#130b1d;
  --floorPx:70px; --platformH:30px;
  --playerX:150px; --playerW:170px; --playerDrop:1px;
  --buddyX:265px; --buddyW:115px; --buddyDrop:13px;
  --skyY:center; --hillsH:445px; --hillsBottom:82px; --groundH:757px; --groundBottom:-120px;
  --tokenLift:126px; --hazardLift:58px; --gameSpeed:3.05; --spawnGap:1450;
  --zSky:0; --zHills:1; --zGround:39; --zPlatform:0; --zBuddy:46; --zPlayer:58; --zBuildings:25; --zObjects:50;
}
/* EDIT SECTION 04: PAGE RESET, GAME CANVAS, SKY, HILLS, GROUND, FLOOR AND HUD */
*{box-sizing:border-box} html,body{margin:0;width:100%;height:100%;overflow:hidden;background:#101022;color:white;font-family:Arial,Helvetica,sans-serif} button,input,select,textarea{font-family:inherit} button{cursor:pointer}
#game{
  position:fixed;
  left:0;
  top:0;
...
.layer{position:absolute;left:0;width:100%;pointer-events:none;background-repeat:repeat-x}.sky{inset:0;z-index:var(--zSky);background-image:url('https://res.cloudinary.com/dpwlfmhia/image/upload/v1781638231/day_sky.png');background-size:cover;background-position:center var(--skyY);background-repeat:no-repeat}.hills{z-index:var(--zHills);height:var(--hillsH);bottom:var(--hillsBottom);background-image:url('https://res.cloudinary.com/dpwlfmhia/image/upload/day_hills_cut.png'),url('https://res.cloudinary.com/dpwlfmhia/image/upload/day_hills.png');background-size:auto 100%,auto 100%;background-position:0 bottom,0 bottom}.groundDecor{z-index:var(--zGround);height:var(--groundH);bottom:var(--groundBottom);background-image:url('https://res.cloudinary.com/dpwlfmhia/image/upload/day_ground_cut.png'),url('https://res.cloudinary.com/dpwlfmhia/image/upload/day_ground.png');background-size:auto 100%,auto 100%;background-position:0 bottom,0 bottom}.underFloor{position:absolute;left:0;right:0;bottom:0;height:calc(var(--floorPx) - var(--platformH));z-index:2;background:#204884;pointer-events:none}.platform{position:absolute;left:0;right:0;bottom:calc(var(--floorPx) - var(--platformH));height:var(--platformH);z-index:var(--zPlatform);pointer-events:none;background:linear-gradient(to bottom,rgba(255,255,255,.92) 0 6px,rgba(210,130,194,1) 6px 22px,rgba(118,199,216,1) 22px 28px,rgba(32,72,132,1) 28px 100%);box-shadow:0 -6px 0 rgba(255,255,255,.16) inset,0 -18px 45px rgba(0,0,0,.14) inset}
/* Night mode: keep the SAME hill/ground artwork and sizing as day mode.
   The separate night_hills/night_ground files are different crops, which made the layers look broken. */
body.night #game{background:#12113b}
body.night .sky{background-image:url('https://res.cloudinary.com/dpwlfmhia/image/upload/v1781638223/night_sky.png')}
body.night .hills{
  background-image:url('https://res.cloudinary.com/dpwlfmhia/image/upload/day_hills_cut.png'),url('https://res.cloudinary.com/dpwlfmhia/image/upload/day_hills.png');
  filter:brightness(.52) saturate(.95) hue-rotate(155deg);
}
body.night .groundDecor{
  background-image:url('https://res.cloudinary.com/dpwlfmhia/image/upload/day_ground_cut.png'),url('https://res.cloudinary.com/dpwlfmhia/image/upload/day_ground.png');
  filter:brightness(.58) saturate(.95) hue-rotate(155deg);
}
body.night .platform{background:linear-gradient(to bottom,rgba(255,255,255,.82) 0 6px,rgba(168,88,190,1) 6px 22px,rgba(92,95,189,1) 22px 28px,rgba(22,42,95,1) 28px 100%)}
body.night .underFloor{background:#14275d}
.hud{position:absolute;z-index:80;top:calc(env(safe-area-inset-top) + 10px);left:50%;transform:translateX(-50%);width:1160px;min-height:58px;display:flex;justify-content:space-between;align-items:center;gap:10px;padding:8px 10px;background:rgba(29,20,44,.78);border:1px solid rgba(255,255,255,.18);border-radius:18px;backdrop-filter:blur(10px);box-shadow:0 10px 26px rgba(0,0,0,.24)}.title{display:flex;flex-direction:column;gap:1px;min-width:250px}.title strong{font-size:clamp(16px,2vw,24px);line-height:1.05}.title span{font-size:12px;opacity:.82}.stats{display:flex;gap:7px;align-items:center;justify-content:flex-end;flex-wrap:wrap}.pill{border:0;border-radius:999px;padding:8px 12px;font-size:14px;line-height:1;font-weight:900;color:#24152e;background:rgba(255,255,255,.88);box-shadow:0 5px 13px rgba(0,0,0,.16)}.gold{background:linear-gradient(180deg,#fff7bd,var(--yellow))}.pink{background:linear-gradient(180deg,#fff1fb,var(--pink))}.blue{background:linear-gradient(180deg,#e9fbff,var(--blue))}.purple{background:linear-gradient(180deg,#fff4ff,#2a1535);color:white}.active{outline:3px solid rgba(245,197,66,.45);background:var(--yellow)}
/* EDIT SECTION 05: CHARACTERS, MISSING SPRITE WARNING, TOKENS, HAZARDS AND COLLECT EFFECTS */
.character{position:absolute;bottom:var(--floorPx);display:flex;align-items:flex-end;justify-content:center;transform:translateX(-50%);pointer-events:none}.character img{max-width:100%;max-height:100%;object-fit:contain;object-position:center bottom;transform-origin:center bottom;filter:drop-shadow(0 22px 18px rgba(0,0,0,.34))}.character.spriteMissing::after{content:attr(data-missing);position:absolute;left:50%;bottom:12px;transform:translateX(-50%);min-width:120px;text-align:center;padding:7px 9px;border-radius:999px;background:rgba(245,197,66,.92);color:#21142d;font-size:12px;font-weight:900;box-shadow:0 10px 20px rgba(0,0,0,.25)}body:not(.editMode) .character.spriteMissing::after{content:'sprite missing';font-size:10px;opacity:.82}#player{z-index:var(--zPlayer);left:var(--playerX);width:clamp(130px,var(--playerW),420px);height:clamp(210px,calc(var(--playerW) * 1.62),660px)}#buddy{z-index:var(--zBuddy);left:var(--buddyX);width:clamp(90px,var(--buddyW),420px);height:clamp(150px,calc(var(--buddyW) * 1.62),660px);opacity:.92;display:none}body.teamMode #buddy{display:flex}#shadow,#buddyShadow{position:absolute;bottom:calc(var(--floorPx) - 12px);height:22px;border-radius:50%;background:rgba(0,0,0,.32);filter:blur(8px);transform:translateX(-50%);pointer-events:none}#shadow{z-index:calc(var(--zPlayer) - 1);left:var(--playerX);width:clamp(100px,13vw,205px)}#buddyShadow{z-index:calc(var(--zBuddy) - 1);left:var(--buddyX);width:clamp(70px,9vw,140px);opacity:.75;display:none}body.teamMode #buddyShadow{display:block}
#buildingObjects{position:absolute;inset:0;z-index:var(--zBuildings);pointer-events:none}
#worldObjects{position:absolute;inset:0;z-index:var(--zObjects);pointer-events:none}.obj{position:absolute;display:grid;place-items:center;width:clamp(46px,5.1vw,68px);height:clamp(46px,5.1vw,68px);border-radius:20px;font-size:clamp(23px,2.8vw,33px);transform:translate(-50%,-50%);border:3px solid rgba(255,255,255,.62);box-shadow:0 12px 20px rgba(0,0,0,.22)}.token{background:linear-gradient(180deg,#fff8ff,#f4c8ee);border-color:var(--pink)}.hazard{background:linear-gradient(180deg,#fff0bf,#ff8585);border-color:#ff5c5c}.finish{background:linear-gradient(180deg,#eafcff,#76C7D8);border-color:#fff}.obj img{display:block;max-width:100%;max-height:100%;width:100%;height:100%;object-fit:contain;pointer-events:none}.obj.hasImage{background:rgba(255,255,255,.66)}.obj.emojiFallback{font-size:clamp(23px,2.8vw,33px)}.obj.collected{transform:translate(-50%,-50%) translateY(-54px) scale(.18);opacity:0;transition:transform .18s ease-out,opacity .18s ease-out}.obj.stomped{transform:translate(-50%,-50%) translateY(18px) scaleY(.18);opacity:.12;transition:transform .2s ease-out,opacity .2s ease-out}.obj.damaged{transform:translate(-50%,-50%) translateX(-36px) rotate(-16deg) scale(.35);opacity:0;transition:transform .2s ease-out,opacity .2s ease-out}.task{background:linear-gradient(180deg,#e9fbff,#b8f4ff);border-color:#76C7D8}.sign{background:linear-gradient(180deg,#fff7bd,#F5C542);border-color:#fff}

/* EDIT SECTION 05B: BIG BUILDINGS + FULL SCREEN DOOR
   Use kind:'building' in the level plan for places like Pet Shop and Habitat.
   These are big entrance buildings. They sit on the floor line and stay above the ground artwork so they do not get buried.
*/
.building{
  width:clamp(215px,23vw,340px)!important;
  height:clamp(155px,18vw,260px)!important;
  ...
  /* Buildings sit on the game floor as entrance scenery, not tiny icons. */
...
.building img{
  ...
  /* Gameplay glow: visible during the actual game, not just Build/Edit. */
  filter:
    drop-shadow(0 0 8px rgba(210,130,194,.95))
    drop-shadow(0 0 16px rgba(118,199,216,.88))
    drop-shadow(0 0 26px rgba(210,130,194,.55))
    drop-shadow(0 18px 20px rgba(0,0,0,.30))!important;
}
.building:hover{transform:translate(-50%,-100%) scale(1.025)!important}

.doorStop{pointer-events:auto;cursor:pointer}
...
#doorScreen.doorPlay #doorBigImg{animation:bigDoorOpen .75s ease-in-out forwards}
@keyframes bigDoorOpen{
  0%{transform:scale(1);opacity:1}
  45%{transform:scale(1.08);opacity:1}
  100%{transform:scale(.82) translateY(-40px);opacity:0}
}
.shopGrid{
  display:grid;
...
.shopCard strong{display:block;font-size:18px;margin-bottom:4px}
.shopCard span{font-size:13px;opacity:.9}
@media(max-width:720px){.shopGrid{grid-template-columns:1fr}.building{width:190px!important;height:140px!important}}


/* EDIT SECTION 05C: SIMPLE BUILDING / PLACE EDITING
   Glow follows the building image shape. It is visible during normal gameplay and stronger when near.
*/
body.editMode #buildingObjects{pointer-events:auto!important;z-index:95!important}

body.editMode .building{
  pointer-events:auto!important;
  cursor:grab;
...
body.editMode .building:active{cursor:grabbing}

/* The glow is on the PNG image itself, so it wraps the actual shop/habitat shape. */
body.editMode .building img{
  filter:
    drop-shadow(0 0 7px rgba(210,130,194,1))
    drop-shadow(0 0 15px rgba(118,199,216,.95))
    drop-shadow(0 0 28px rgba(210,130,194,.75))
    drop-shadow(0 18px 20px rgba(0,0,0,.30))!important;
}

body.editMode .building.selectedPlaceTarget{
  outline:none!important;
  border:0!important;
  box-shadow:none!important;
}
body.editMode .building.selectedPlaceTarget img{
  filter:
    drop-shadow(0 0 10px rgba(118,199,216,1))
    drop-shadow(0 0 22px rgba(210,130,194,1))
    drop-shadow(0 0 38px rgba(118,199,216,.9))
    drop-shadow(0 0 58px rgba(210,130,194,.65))
    drop-shadow(0 18px 20px rgba(0,0,0,.30))!important;
  animation:waaBuildingImageGlowPulse 1s ease-in-out infinite alternate;
}

body.editMode .building.editorPreview{
  border:0!important;
  box-shadow:none!important;
}
body.editMode .building.editorPreview img{
  filter:
    drop-shadow(0 0 8px rgba(210,130,194,1))
    drop-shadow(0 0 18px rgba(118,199,216,.9))
    drop-shadow(0 18px 20px rgba(0,0,0,.30))!important;
}

/* Normal game: light building glow only.
   The older version used several huge drop-shadows plus a pulse animation when the
   player came near a building. That made the sprites feel laggy/draggy around shops.
*/
body:not(.editMode) .building{
  outline:none!important;
  border:0!important;
  box-shadow:none!important;
  will-change:transform;
}
body:not(.editMode) .building img{
  filter:drop-shadow(0 14px 16px rgba(0,0,0,.28))!important;
  transform:translateZ(0);
  backface-visibility:hidden;
}
body:not(.editMode) .building.buildingNearby img{
  /* Keep a clear "you can enter" cue, but do not animate or use heavy glow filters. */
  filter:drop-shadow(0 0 10px rgba(118,199,216,.92)) drop-shadow(0 16px 16px rgba(0,0,0,.30))!important;
  animation:none!important;
}
@keyframes waaBuildingGameGlowPulse{
  from{transform:none}
  to{transform:none}
}

/* Keep task/sign glow separate; buildings no longer use square box glow. */
.obj.interactNearby{
  outline:none!important;
  border-color:rgba(118,199,216,1)!important;
  box-shadow:0 0 0 3px rgba(210,130,194,.86),0 0 14px rgba(118,199,216,.80)!important;
}

@keyframes waaBuildingImageGlowPulse{
  from{
    filter:
      drop-shadow(0 0 8px rgba(118,199,216,.9))
      drop-shadow(0 0 18px rgba(210,130,194,.9))
      drop-shadow(0 0 28px rgba(118,199,216,.55))
      drop-shadow(0 18px 20px rgba(0,0,0,.30));
  }
  to{
    filter:
      drop-shadow(0 0 14px rgba(118,199,216,1))
      drop-shadow(0 0 30px rgba(210,130,194,1))
      drop-shadow(0 0 52px rgba(118,199,216,.9))
      drop-shadow(0 18px 20px rgba(0,0,0,.30));
  }
}
.placeEditorGrid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:8px 0}.placeEditorGrid label{font-size:12px;font-weight:900}.placeEditorGrid select{margin-top:4px}
.layerHelp{font-size:12px;line-height:1.35;background:rgba(118,199,216,.12);border:1px solid rgba(118,199,216,.42);border-radius:12px;padding:8px;margin:6px 0 8px}



/* EDIT SECTION 05D: TUTORIAL STOP BOARDS
   These are now small stop markers placed before the matching buildings, not random block clutter. */
.obj.lessonBlock{
  ...
  color:#24152e!important;
  border:3px solid rgba(255,255,255,.92)!important;
  box-shadow:0 8px 16px rgba(0,0,0,.20),0 0 12px rgba(118,199,216,.35)!important;
}
.obj.lessonBlock::before{
  content:attr(data-icon);
  display:block;
  font-size:20px;
  line-height:1;
}
.obj.lessonBlock::after{
...
.obj.sign.lessonBlock{background:linear-gradient(180deg,#fff7bd,#F5C542)!important;border-color:#fff!important;}
.obj.task.lessonBlock{background:linear-gradient(180deg,#e9fbff,#b8f4ff)!important;border-color:#76C7D8!important;}
body:not(.editMode) .obj.lessonBlock.interactNearby{
  box-shadow:0 0 0 3px rgba(210,130,194,.95),0 0 20px rgba(118,199,216,1),0 0 34px rgba(245,197,66,.7)!important;
}

/* EDIT SECTION 06: MENUS, BUILDER, PET CARE, MOBILE CONTROLS AND TAMAGOTCHI STYLE */
.toast{position:absolute;z-index:95;left:50%;top:calc(env(safe-area-inset-top) + 86px);bottom:auto;transform:translateX(-50%) translateY(-12px);opacity:0;background:linear-gradient(180deg,#fff7bd,var(--yellow));color:#21142b;padding:10px 16px;border-radius:999px;font-weight:900;text-align:center;max-width:min(88vw,720px);box-shadow:0 14px 28px rgba(0,0,0,.28);transition:.18s}.toast.show{opacity:1;transform:translateX(-50%) translateY(0)}.instructions{position:absolute;z-index:79;right:14px;top:calc(env(safe-area-inset-top) + 82px);max-width:330px;padding:12px 14px;border-radius:18px;background:rgba(29,20,44,.72);border:1px solid rgba(255,255,255,.18);backdrop-filter:blur(9px)}.instructions h2{font-size:17px;margin:0 0 5px}.instructions p{font-size:13px;line-height:1.32;margin:0;opacity:.9}.instructions.hidden{display:none}
.builder,.petPanel{position:absolute;z-index:100;left:14px;top:calc(env(safe-area-inset-top) + 82px);width:min(430px,calc(100vw - 28px));max-height:calc(100dvh - 112px);overflow:auto;padding:12px;border-radius:18px;background:rgba(24,16,36,.94);border:1px solid rgba(255,255,255,.24);backdrop-filter:blur(12px);box-shadow:0 16px 34px rgba(0,0,0,.36)}.petPanel{left:auto;right:14px;background:rgba(255,246,253,.96);color:#24152e;border-color:rgba(210,130,194,.8)}.builder.hidden,.petPanel.hidden{display:none}.builder h2,.petPanel h2{font-size:17px;margin:0 0 6px}.builder p,.petPanel p{font-size:12px;line-height:1.34;margin:0 0 10px;opacity:.86}.builder select,.builder textarea,.builder input,.petPanel input,.petPanel select{width:100%;border:0;border-radius:10px;padding:8px;margin:5px 0 8px}.petPanel input,.petPanel select{border:1px solid #76C7D8}.calRow{display:grid;grid-template-columns:132px 1fr 62px;gap:8px;align-items:center;margin:8px 0;font-size:12px}.calRow input{width:100%}.calButtons{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}.calButtons button,.petPanel button{border:0;border-radius:999px;padding:8px 10px;font-weight:900;color:#21142d;background:rgba(255,255,255,.88)}.calButtons .good,.petPanel .good{background:#22c55e;color:white}.calButtons .warn,.petPanel .warn{background:#F5C542}.calButtons .bad{background:#e11d48;color:white}textarea{min-height:80px;font-family:ui-monospace,SFMono-Regular,Consolas,monospace;font-size:12px}body.editMode .editableTarget{outline:3px dashed rgba(245,197,66,.9);outline-offset:4px}body.editMode .editableTarget{pointer-events:auto!important}body.editMode .character{pointer-events:auto!important}body.editMode .platform{pointer-events:auto!important}body.editMode .layer{pointer-events:auto!important}body.editMode #game{cursor:move}.builder,.petPanel,.hud,.overlay,.touch,.helpBtn{cursor:auto}.dragHint{position:absolute;z-index:105;pointer-events:none;display:none;padding:6px 9px;border-radius:999px;background:rgba(245,197,66,.95);color:#21142d;font-weight:900;font-size:12px}body.editMode .dragHint{display:block}.quickTabs{display:flex;gap:8px;flex-wrap:wrap;margin:8px 0}.quickTabs button.active{background:#76C7D8;color:#102033}.tabBody{display:none}.tabBody.active{display:block}.statusBox{background:rgba(7,19,38,.08);border:1px solid rgba(118,199,216,.55);border-radius:10px;padding:8px;font-size:12px;white-space:pre-wrap}.petPreview{height:170px;border:1px dashed #d282c2;border-radius:14px;display:grid;place-items:center;margin:8px 0;overflow:hidden;background:#fff}.petPreview img{max-width:100%;max-height:100%;object-fit:contain}.petPreview.empty{color:#594164}

/* Tidy editor slide-out + robust PetBot loading */
.hudMenu{background:linear-gradient(180deg,#fef7ff,#d282c2);color:#24152e}
.panelHead{display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:8px}.panelHead h2{margin:0}.miniClose{border:0;border-radius:999px;padding:8px 10px;font-weight:900;background:#F5C542;color:#21142d}.builder{width:min(520px,calc(100vw - 28px))}.builderHelp{background:rgba(118,199,216,.13);border:1px solid rgba(118,199,216,.45);border-radius:14px;padding:9px 10px;margin:8px 0 10px;font-size:12px;line-height:1.35}.editPickGrid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:8px 0 10px}.targetPick{border:0;border-radius:14px;padding:10px;font-weight:900;background:rgba(255,255,255,.87);color:#21142d}.targetPick.active{background:#76C7D8;color:#102033;outline:3px solid rgba(245,197,66,.45)}.editSection{border:1px solid rgba(255,255,255,.14);border-radius:14px;padding:10px;margin:9px 0;background:rgba(255,255,255,.06)}.editSection h3{margin:0 0 8px;font-size:13px;color:#F5C542}.selectedEditTarget{outline:4px dashed rgba(118,199,216,.95)!important;outline-offset:8px!important}.tabHint{font-size:12px;opacity:.84;margin:0 0 8px}.menuModal .choiceRow button.secondary{background:#76C7D8}.menuModal .choiceRow button.dark{background:#2a1535;color:#fff}.menuModal .choiceRow button.good{background:#22c55e;color:#fff}
.petPanel{width:min(480px,calc(100vw - 28px));padding:14px;background:linear-gradient(180deg,#fff7fd,#ffe7f7);border:3px solid #d282c2;color:#24152e}.tamaShell{border-radius:28px;padding:14px;background:linear-gradient(180deg,#ffcfef,#d282c2);box-shadow:inset 0 -12px 0 rgba(0,0,0,.08),0 16px 34px rgba(0,0,0,.28)}.tamaTop{display:flex;justify-content:space-between;align-items:center;gap:8px}.tamaTop h2{font-size:20px}.tamaScreen{min-height:220px;margin:12px 0;padding:12px;border-radius:20px;background:linear-gradient(180deg,#eafdff,#ffffff);border:4px solid #76C7D8;display:grid;grid-template-columns:170px 1fr;gap:12px;align-items:center}.petPreview{height:170px;margin:0;border:2px dashed #d282c2;background:#fff;border-radius:18px}.petInfo h3{margin:0 0 2px;font-size:22px;color:#24152e}.petInfo p{margin:0 0 8px}.petBadges{display:flex;gap:6px;flex-wrap:wrap;margin:6px 0}.petBadge{display:inline-flex;border-radius:999px;padding:5px 8px;background:#f1d7ec;font-size:11px;font-weight:900}.petBadge.green{background:#bbf7d0}.petBadge.blue{background:#bfedf6}.petStats label{display:block;margin:7px 0 3px;font-size:12px;font-weight:900}.bar{height:13px;border-radius:999px;background:#ead7e9;overflow:hidden}.bar span{display:block;height:100%;width:0%;border-radius:999px;background:linear-gradient(90deg,#76C7D8,#22c55e)}.tamaControls{display:grid;grid-template-columns:1fr 1fr;gap:8px}.tamaControls button{border:0;border-radius:999px;padding:10px;font-weight:900;background:#fff;color:#24152e}.tamaControls .good{background:#22c55e;color:#fff}.tamaControls .warn{background:#F5C542}.petFormGrid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px}.petFormGrid label{font-size:12px;font-weight:900}.petFormGrid input,.petFormGrid select{margin-top:3px}.petStatusLine{margin-top:8px;background:rgba(255,255,255,.62);border:1px solid rgba(118,199,216,.55);color:#24152e}.petNoPets{font-weight:900;color:#8a315f}

/* Editor cleanup: right-side slide panel, one section at a time */
.builder{left:auto!important;right:14px!important;width:min(390px,calc(100vw - 28px))!important;background:rgba(23,17,35,.96)!important;color:#fff;max-height:calc(100dvh - 100px)!important;padding:10px!important}
.builder .builderHelp{font-size:11px;padding:8px;margin:6px 0 8px}.builder h2{font-size:16px}.quickTabs{position:sticky;top:0;background:rgba(23,17,35,.96);z-index:2;padding:4px 0}.editPickGrid{grid-template-columns:1fr 1fr;gap:6px}.targetPick{padding:8px;font-size:12px}.calRow{grid-template-columns:104px 1fr 52px;font-size:11px;gap:6px;margin:6px 0}.builder .editSection{display:none}.builder[data-target="player"] .section-player,.builder[data-target="buddy"] .section-buddy,.builder[data-target="hills"] .section-layers,.builder[data-target="ground"] .section-layers,.builder[data-target="platform"] .section-layers,.builder[data-target="sky"] .section-layers,.builder[data-target="objects"] .section-objects{display:block}.panelHead{position:sticky;top:0;background:rgba(23,17,35,.97);z-index:3;padding-bottom:4px}.petStatusLine{min-height:34px;display:block}.tamaScreen.loading{opacity:.75}.tamaScreen.loading .petPreview{background:linear-gradient(90deg,#fff,#eafdff,#fff);background-size:200% 100%;animation:tamaLoad 1.1s linear infinite}@keyframes tamaLoad{to{background-position:-200% 0}}
@media(max-width:820px){.builder{left:8px!important;right:8px!important;width:auto!important;max-height:68vh!important}.builder .builderHelp{display:none}.editPickGrid{grid-template-columns:1fr 1fr}.calRow{grid-template-columns:94px 1fr 48px}}

@media(max-width:820px){.builder,.petPanel{width:auto}.tamaScreen{grid-template-columns:1fr}.petFormGrid{grid-template-columns:1fr}.editPickGrid{grid-template-columns:1fr 1fr}.tamaControls{grid-template-columns:1fr 1fr}}

.overlay{position:absolute;inset:0;z-index:120;display:grid;place-items:center;background:rgba(12,7,20,.38);backdrop-filter:blur(5px)}.overlay.hidden{display:none}.modal{width:min(680px,calc(100vw - 26px));padding:24px;text-align:center;background:rgba(36,24,55,.9);border:1px solid rgba(255,255,255,.22);border-radius:26px;box-shadow:0 18px 40px rgba(0,0,0,.38)}.modal h1{margin:0 0 8px;font-size:clamp(28px,5vw,48px)}.modal p{margin:0 0 16px;line-height:1.45}.modal button{border:0;border-radius:999px;padding:12px 22px;margin:4px;background:var(--yellow);color:#21142d;font-size:18px;font-weight:900}.choiceRow{display:flex;gap:10px;justify-content:center;flex-wrap:wrap}
.touch{position:absolute;z-index:90;left:14px;bottom:calc(env(safe-area-inset-bottom) + 14px);display:none;grid-template-columns:60px 60px 60px;gap:8px}.touch button{height:56px;border:0;border-radius:16px;font-size:24px;font-weight:900;color:#21142d;background:rgba(255,255,255,.82)}.touch .wide{grid-column:span 3;background:linear-gradient(180deg,#fff7bd,var(--yellow))}.helpBtn{position:absolute;z-index:90;right:14px;bottom:calc(env(safe-area-inset-bottom) + 14px)}.hitFlash{animation:shake .2s linear}@keyframes shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-7px)}75%{transform:translateX(7px)}}
@media(max-width:820px){.instructions{display:none}.touch{display:grid}}
@media(min-width:821px){.touch{display:none}}

/* Clean selected-only editor + Tamagotchi restore + direction fixes */
.builder .editSection{display:none!important}
.builder .calRow{display:none}

/* BUILDING SIZE EDITOR FIX:
   Place/building sliders must stay visible, including the new Height slider. */
.builder.show-places .section-places .calRow{
  display:grid!important;
}
.builder.show-player .section-player,
.builder.show-buddy .section-buddy,
.builder.show-world .section-layers,
.builder.show-objects .section-objects,
.builder.show-places .section-places,
.builder.show-layers .section-layers{display:block!important}
.petPanel{background:transparent!important;border:0!important;box-shadow:none!important;padding:10px!important}
.tamaShell{border:5px solid #d282c2!important;border-radius:38px!important;background:radial-gradient(circle at 50% 0%,#fff3fb 0 18%,#ffc7ea 19% 45%,#d282c2 46% 100%)!important;box-shadow:inset 0 -16px 0 rgba(92,39,93,.18),0 20px 45px rgba(0,0,0,.32)!important;}
.tamaScreen{border:5px solid #76C7D8!important;border-radius:24px!important;background:linear-gradient(180deg,#eaffff,#ffffff)!important;min-height:230px!important}
.petPreview{background:#fff!important;border:3px dashed #d282c2!important}
.petSizeControls{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:8px 0}.petSizeControls label{font-size:12px;font-weight:900}.petSizeControls input,.petSizeControls select{margin-top:4px}
.character img{transition:none!important}
/* STAGE 26 WALK SMOOTHING: keep the moving sprites on the GPU and avoid transition stutter. */
.character,.character img,#shadow,#buddyShadow,#teamShadow2,#teamShadow3{will-change:transform;backface-visibility:hidden;}
.character img{transform:translateZ(0);}
.character.spriteMissing img{opacity:1!important;visibility:visible!important;}


/* C: TEAM OF 4 FOLLOWERS
   Solo mode shows one character. Team mode shows four: main + 3 followers.
   These are visual followers; the main character still controls jumping/collisions.
*/
#teamMate2,#teamMate3{
  z-index:calc(var(--zBuddy) - 2);
  width:clamp(82px,120px,260px);
  height:clamp(135px,195px,430px);
  opacity:.88;
  display:none;
}
#teamMate2{left:7vw;}
#teamMate3{left:34vw;}
body.team4Mode #teamMate2,
body.team4Mode #teamMate3{display:flex;}
#teamShadow2,#teamShadow3{
  position:absolute;
  bottom:calc(var(--floorPx) - 12px);
  height:18px;
  border-radius:50%;
  ...
  pointer-events:none;
  z-index:calc(var(--zBuddy) - 3);
  display:none;
}
#teamShadow2{left:7vw;width:88px;}
#teamShadow3{left:34vw;width:96px;}
body.team4Mode #teamShadow2,
body.team4Mode #teamShadow3{display:block;}
@media(max-width:820px){#teamMate2{left:7vw;width:82px;height:136px}#teamMate3{left:48vw;width:88px;height:144px}#teamShadow2{left:7vw;width:62px}#teamShadow3{left:48vw;width:68px}}

/* D: TEAM PICKER + FLEXIBLE TEAM SLOTS
   Team mode can now use 1, 2, 3 or 4 selected characters.
   Extra followers are hidden unless the player picked them.
*/
.teamPickerGrid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin:8px 0 14px;text-align:left}.teamPickerGrid label,.soloPickerBox label{font-weight:900;font-size:13px}.teamPickerGrid select,.soloPickerBox select{width:100%;border:0;border-radius:14px;padding:10px;margin-top:4px;font-weight:900}.teamPickerBox,.soloPickerBox{margin:10px auto 12px;max-width:560px}.teamPickerBox p{font-size:13px;margin:0 0 8px;opacity:.9}body.teamExtra2 #teamMate2{display:flex}body.teamExtra3 #teamMate3{display:flex}body.teamExtra2 #teamShadow2{display:block}body.teamExtra3 #teamShadow3{display:block}@media(max-width:820px){.teamPickerGrid{grid-template-columns:1fr}}


/* B: AUTO CHARACTER SIZE RULE
   Main/side slots no longer decide character size.
   JavaScript CHARACTER_CONFIGS decides each character's correct size.
   Keep #player and #buddy as physics boxes; scale #sprite and #buddySprite only.
*/

/* A: INVISIBLE FLOOR FIX
   This keeps the floor physics working but hides the coloured platform stripe.
   Do not delete .platform or .underFloor — they help the characters stand in the right place.
*/
.underFloor,
.platform{
  background:transparent!important;
...
body.night .underFloor,
body.night .platform{
  background:transparent!important;
  box-shadow:none!important;
}
/* MOBILE CLEAN GAME VIEW */
@media(max-width:820px){
  ...
  .touch{
    ...
    grid-template-columns:92px 92px 92px!important;
    gap:14px!important;
  }
...
/* CLEAN START MENU FIX */
.menuModal{
  width:min(760px,calc(100vw - 32px))!important;
  padding:26px!important;
  border-radius:28px!important;
  background:rgba(31,22,50,.94)!important;
...
body.axFrameDebugOn .axFrameDebug{display:block!important}
body.axFrameDebugOn #axFrameDebug{display:block!important}



/* UX PATCH: clearer editor groups, stronger missing-sprite fallback, readable debug and pet bars */
.editorGroupHeader{
  grid-column:1/-1;
  margin:8px 0 2px;
  padding:7px 9px;
  border-radius:12px;
  background:rgba(118,199,216,.16);
  border:1px solid rgba(118,199,216,.42);
  color:#F5C542;
  font-size:12px;
  font-weight:900;
  letter-spacing:.03em;
...
body.editMode .targetPick{min-height:42px}
.character.spriteMissing::before{
  content:'';
  position:absolute;
  left:50%;
  bottom:0;
  ...
  height:92%;
  transform:translateX(-50%);
  border:4px dashed rgba(118,199,216,.95);
  border-radius:24px;
  box-shadow:0 0 0 3px rgba(245,197,66,.65),0 0 24px rgba(210,130,194,.8);
  pointer-events:none;
}
.axFrameDebug{
...
#hungerBar{background:linear-gradient(90deg,#ef4444,#F5C542)!important}
#happyBar{background:linear-gradient(90deg,#d282c2,#22c55e)!important}
#energyBar{background:linear-gradient(90deg,#76C7D8,#3b82f6)!important}
.bar{position:relative;height:16px!important}
.bar span{min-width:0;transition:width .18s ease-out}
.layerStackHint{
...
/* PETBOT TAMAGOTCHI DOCK + FULL SCREEN
   Small mode is now only the toy PNG on top of the game: no iframe background.
   Full screen mode loads /petbot_test.html for the working app. */
.petbotDock{
  position:fixed;
  z-index:92;
...
.petbotDock.hidden{display:none;}
.petbotDock.fullscreen{
  inset:0!important;
  width:auto!important;
  height:auto!important;
...
.petbotDockPreview{
  ...
  place-items:center;
  pointer-events:none;
  filter:drop-shadow(0 12px 18px rgba(0,0,0,.28)) drop-shadow(0 0 12px rgba(118,199,216,.45));
}
.petbotDockPreview img{
  width:100%;
  height:100%;
  object-fit:contain;
  display:block;
...
.petbotDock.fullscreen .petbotDockFrame{display:block;}
.petbotDock.fullscreen .petbotDockPreview{display:none;}
.petbotDockBar{
  position:absolute;
  z-index:2;
  left:8px;
...
.petbotDock.fullscreen .petbotDockBar{display:flex;}
.petbotDockBtn{
  pointer-events:auto;
  border:0;
  border-radius:999px;
...
.petbotDockBtn.main{background:linear-gradient(180deg,#fff,#f8edf6)}
.petbotDockHint{
  position:absolute;
  z-index:2;
  left:50%;
...
.petbotDock.fullscreen .petbotDockHint{display:none;}
@media(max-width:900px){
  .petbotDock{width:128px;height:168px;right:10px;top:80px;}
}
@media(max-width:520px){
  .petbotDock{width:96px;height:130px;top:74px;right:7px;}
  .petbotDockHint{display:none;}
}


/* FINAL TAMAGOTCHI DOCK FIX
   Only one visible Tamagotchi control: the HUD button plus this small toy preview.
   The dock is moved away from the top/right edge so it is not chopped off. */
#petbotDock:not(.fullscreen){
  position:fixed!important;
  top:calc(env(safe-area-inset-top) + 110px)!important;
  right:calc(env(safe-area-inset-right) + 22px)!important;
  width:clamp(126px,8.5vw,158px)!important;
  height:clamp(166px,11.5vw,214px)!important;
...
#petbotDock:not(.fullscreen) .petbotDockPreview{
  inset:0!important;
  background:transparent!important;
  border:0!important;
  box-shadow:none!important;
}
#petbotDock:not(.fullscreen) .petbotDockPreview img{
  width:100%!important;
  height:100%!important;
  object-fit:contain!important;
...
#petbotDock:not(.fullscreen) .petbotDockBar,
#petbotDock:not(.fullscreen) .petbotDockHint{
  display:none!important;
}
#petbotDock.fullscreen .petbotDockBar{
  display:flex!important;
}
@media(max-width:900px){
  #petbotDock:not(.fullscreen){top:96px!important;right:14px!important;width:118px!important;height:158px!important;min-width:118px!important;min-height:158px!important;}
}
@media(max-width:520px){
  #petbotDock:not(.fullscreen){top:82px!important;right:8px!important;width:88px!important;height:118px!important;min-width:88px!important;min-height:118px!important;}
}




/* TAMAGOTCHI DOCK FINAL POSITION + MINI SCREEN
   Background stays transparent, but the toy screen is filled so it does not look hollow. */
#petbotDock:not(.fullscreen){
  top:calc(env(safe-area-inset-top) + 260px)!important;
  right:calc(env(safe-area-inset-right) + 32px)!important;
...
#petbotDock:not(.fullscreen) .petbotDockMiniLcd{
  position:absolute!important;
  z-index:1!important;
  left:23.4%!important;
  top:38.4%!important;
  ...
  background:linear-gradient(180deg,#eff8ff,#bfe1ff 56%,#8fc7ff)!important;
  box-shadow:inset 0 0 8px rgba(0,0,0,.20), inset 0 0 0 1px rgba(30,54,90,.28)!important;
...
#petbotDock:not(.fullscreen) .petbotDockMiniLcd::before{
  content:'';
  position:absolute;
  inset:0;
  background-image:linear-gradient(rgba(20,30,50,.055) 1px,transparent 1px),linear-gradient(90deg,rgba(20,30,50,.055) 1px,transparent 1px);
  background-size:4px 4px;
  opacity:.38;
}
#petbotDock:not(.fullscreen) .petbotDockMiniLcd .miniTitle{
  position:absolute;
  top:7px;
  left:0;
  right:0;
...
#petbotDock:not(.fullscreen) .petbotDockMiniLcd .miniPet{
  position:relative;
  z-index:1;
  width:58%;
  height:58%;
...
#petbotDock:not(.fullscreen) .petbotDockMiniLcd .miniPet img{
  max-width:100%;
  max-height:100%;
  object-fit:contain;
  display:block;
}
#petbotDock:not(.fullscreen) .petbotDockMiniLcd .miniPet span{
  font-size:18px;
  line-height:1;
}
#petbotDock:not(.fullscreen) .petbotDockMiniLcd .miniBars{
  position:absolute!important;
  z-index:2!important;
  left:15%!important;
  right:15%!important;
...
#petbotDock:not(.fullscreen) .petbotDockMiniLcd .miniBars i{
  display:block!important;
  flex:1 1 0!important;
  height:4px!important;
  border-radius:999px!important;
  background:linear-gradient(90deg,#76C7D8,#d282c2)!important;
  box-shadow:inset 0 0 0 1px rgba(25,40,70,.22)!important;
...
@media(max-width:900px){
  #petbotDock:not(.fullscreen){top:210px!important;right:18px!important;width:132px!important;height:176px!important;min-width:132px!important;min-height:176px!important;}
}
@media(max-width:520px){
  #petbotDock:not(.fullscreen){top:150px!important;right:10px!important;width:98px!important;height:132px!important;min-width:98px!important;min-height:132px!important;}
}



/* PETBOT FULLSCREEN BACK BUTTON FIX
   This button sits outside the iframe, so it can always close the Tamagotchi and return to the game. */
.petbotBackBtn{
  display:none;
  position:fixed;
  z-index:10060;
  ...
  border-radius:999px;
  padding:12px 16px;
  background:linear-gradient(180deg,#fff,#f8edf6);
  color:#25172e;
  font-weight:1000;
  font-size:16px;
  line-height:1;
  box-shadow:0 10px 26px rgba(0,0,0,.24),0 0 0 4px rgba(210,130,194,.16);
  cursor:pointer;
  pointer-events:auto;
}
body.petbotOpen .petbotBackBtn{display:block!important;}
body.petbotOpen #petbotDock.fullscreen .petbotDockBar{
  display:flex!important;
  position:fixed!important;
...
body.petbotOpen #petbotExpandBtn{font-size:0!important;}
body.petbotOpen #petbotExpandBtn::after{content:'Back';font-size:15px!important;}
@media(max-width:520px){
  .petbotBackBtn{top:calc(env(safe-area-inset-top) + 10px);left:calc(env(safe-area-inset-left) + 10px);padding:10px 12px;font-size:14px;}
  body.petbotOpen #petbotDock.fullscreen .petbotDockBar{right:10px!important;top:10px!important;}
}



/* TAMAGOTCHI-FIRST LAUNCH
   When the PetBot toy is open, the old HUD/menu does not sit on top of it. */
body.petbotOpen .hud,
body.tamagotchiLanding .hud{display:none!important;}
body.tamagotchiLanding #startScreen{display:none!important;}
body.petbotOpen #startScreen{display:none!important;}
body.petbotOpen #petbotDock.fullscreen{background:radial-gradient(circle at 50% 42%,#fff 0,#fff2fb 44%,#def5ff 100%)!important;}
body.petbotOpen::before{content:"";position:fixed;inset:0;z-index:9998;background:radial-gradient(circle at 50% 42%,#fff 0,#fff2fb 44%,#def5ff 100%);pointer-events:none;}
body.petbotOpen #petbotDock.fullscreen{z-index:10000!important;}

/* EDIT SECTION 06: CSS ENDS HERE - HTML STRUCTURE STARTS NEXT */


/* CLEAN TAMAGOTCHI FULLSCREEN FIX 20260628
   No floating web controls while the Tamagotchi is open. The toy itself is the controller. */
body.petbotOpen .petbotBackBtn,
body.petbotOpen #petbotDock.fullscreen .petbotDockBar{
  display:none!important;
}
body.petbotOpen .selectedEditTarget,
body.petbotOpen .editableTarget,
body.petbotOpen .character,
body.petbotOpen .building,
body.petbotOpen .obj{
  outline:none!important;
}
body.petbotOpen .dragHint,
body.petbotOpen .axFrameDebug,
body.petbotOpen .character.spriteMissing::before,
body.petbotOpen .character.spriteMissing::after{
  display:none!important;
}
body.petbotOpen #petbotDock.fullscreen .petbotDockFrame{
  display:block!important;
  background:transparent!important;
}
...
/* FIX 20260628B: the blank pastel screen was caused by body.petbotOpen::before
   sitting above the transformed game canvas. The Tamagotchi dock is moved to <body>
   and becomes the full-screen clean background itself. */
body.petbotOpen::before{display:none!important;content:none!important;}
body.petbotOpen #game{visibility:hidden!important;}
body.petbotOpen #petbotDock{visibility:visible!important;}
body.petbotOpen #petbotDock.fullscreen{
  position:fixed!important;
  inset:0!important;
  left:0!important;
  top:0!important;
  ...
  transform:none!important;
  background:radial-gradient(circle at 50% 42%,#fff 0,#fff2fb 44%,#def5ff 100%)!important;
  overflow:hidden!important;
}
body.petbotOpen #petbotDock.fullscreen .petbotDockFrame{
  display:block!important;
...
body.petbotOpen #startScreen,
body.petbotOpen .overlay,
body.petbotOpen .hud,
body.petbotOpen .builder,
body.petbotOpen .petPanel,
body.petbotOpen .instructions,
body.petbotOpen .touch,
body.petbotOpen .helpBtn{display:none!important;}



/* STAGE 7: real game is allowed to sit behind the transparent Tamagotchi LCD first.
   Siren expands it back to the normal full-screen game view. */
body.petbotOpen.tamaGameEmbedded #game{
  visibility:visible!important;
  display:block!important;
  position:fixed!important;
  left:50vw!important;
  top:47.8dvh!important;
  width:1920px!important;
  height:1080px!important;
  min-width:1920px!important;
  min-height:1080px!important;
  transform-origin:center center!important;
  transform:translate(-50%,-50%) scale(.185)!important;
  z-index:2147482999!important;
  pointer-events:none!important;
  border-radius:30px!important;
  overflow:hidden!important;
  box-shadow:0 0 0 4px rgba(22,34,63,.18)!important;
}
body.petbotOpen.tamaGameEmbedded .hud,
body.petbotOpen.tamaGameEmbedded .instructions,
body.petbotOpen.tamaGameEmbedded .touch,
body.petbotOpen.tamaGameEmbedded .helpBtn,
body.petbotOpen.tamaGameEmbedded .builder,
body.petbotOpen.tamaGameEmbedded .overlay,
body.petbotOpen.tamaGameEmbedded #startScreen{display:none!important}
body.petbotOpen.tamaGameEmbedded #petbotDock.fullscreen .petbotDockFrame{background:transparent!important;opacity:1!important}
@media(max-width:700px){
  body.petbotOpen.tamaGameEmbedded #game{transform:translate(-50%,-50%) scale(.15)!important;top:48.3dvh!important}
}

/* STAGE 10: when a quest starts inside the Tamagotchi, the parent dock must not cover the real game. */
body.petbotOpen.tamaGameEmbedded #petbotDock.fullscreen{
  background:transparent!important;
  pointer-events:none!important;
}
body.petbotOpen.tamaGameEmbedded #petbotDock.fullscreen .petbotDockFrame{
  pointer-events:auto!important;
  background:transparent!important;
}
body.petbotOpen.tamaGameEmbedded #game{
...
/* STAGE 12: better LCD embedded game fit and no huge control rings. */
body.petbotOpen.tamaGameEmbedded #game{
  left:50vw!important;
  top:48.8dvh!important;
  transform:translate(-50%,-50%) scale(.170)!important;
  border-radius:22px!important;
  box-shadow:none!important;
  pointer-events:none!important;
}
body.petbotOpen.tamaGameEmbedded #petbotDock.fullscreen{
  filter:drop-shadow(0 18px 30px rgba(18,20,45,.34))!important;
}
@media(max-width:700px){
  body.petbotOpen.tamaGameEmbedded #game{transform:translate(-50%,-50%) scale(.145)!important;top:49.1dvh!important;}
}

/* STAGE 13: fill the Tamagotchi LCD better. The real game is cropped into the LCD instead of tiny letterboxed. */
body.petbotOpen.tamaGameEmbedded #game{
  left:50vw!important;
  top:48.4dvh!important;
  transform:translate(-50%,-50%) scale(.205)!important;
  transform-origin:center center!important;
  border-radius:18px!important;
  box-shadow:none!important;
}
@media(max-width:700px){
  body.petbotOpen.tamaGameEmbedded #game{transform:translate(-50%,-50%) scale(.175)!important;top:48.7dvh!important;}
}


/* Stage 19: CSS fullscreen fallback. Do not rely on browser Fullscreen API. */
body.petbotOpen #petbotDock.fullscreen{position:fixed!important;inset:0!important;width:100vw!important;height:100dvh!important;min-width:100vw!important;min-height:100dvh!important;max-width:none!important;max-height:none!important;transform:none!important;display:grid!important;place-items:center!important;}
body.petbotOpen #petbotDock.fullscreen .petbotDockFrame{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;border:0!important;background:transparent!important;}
body.petbotOpen #petbotDock.fullscreen .petbotDockBar{display:none!important;}
body.gameExpandedFromTama #petbotDock{display:none!important;}
body.gameExpandedFromTama #game{visibility:visible!important;opacity:1!important;display:block!important;z-index:9999!important;}
body.gameExpandedFromTama .overlay,body.gameExpandedFromTama #startScreen{display:none!important;}

/* Stage 21: dynamically fitted embedded game inside the real Tamagotchi LCD. */
body.petbotOpen.tamaGameEmbedded #game{
  left:var(--tama-lcd-cx,50vw)!important;
  top:var(--tama-lcd-cy,50dvh)!important;
  transform:translate3d(-50%,-50%,0) scale(var(--tama-game-scale,.24))!important;
  transform-origin:center center!important;
  border-radius:22px!important;
  box-shadow:none!important;
...
body.petbotOpen.tamaGameEmbedded #petbotDock.fullscreen{background:transparent!important;}
body.petbotOpen.tamaGameEmbedded #petbotDock.fullscreen .petbotDockFrame{background:transparent!important;}


/* Stage 22: LCD game fit repair. The real quest is left-anchored inside the toy screen so the player sprite stays visible. */
body.petbotOpen.tamaGameEmbedded #game{
  left:var(--tama-lcd-cx,50vw)!important;
  top:var(--tama-lcd-cy,50dvh)!important;
  transform:translate3d(-50%,-50%,0) scale(var(--tama-game-scale,.36))!important;
  transform-origin:center center!important;
  border-radius:18px!important;
  box-shadow:none!important;
  pointer-events:none!important;
...
/* Stage 25: hard crop the real quest to the Tamagotchi LCD.
   This stops the road/background drawing outside the toy screen. */
body.petbotOpen.tamaGameEmbedded #game{
  clip-path:inset(var(--tama-clip-top,360px) var(--tama-clip-right,860px) var(--tama-clip-bottom,70px) var(--tama-clip-left,0px) round 26px)!important;
  -webkit-clip-path:inset(var(--tama-clip-top,360px) var(--tama-clip-right,860px) var(--tama-clip-bottom,70px) var(--tama-clip-left,0px) round 26px)!important;
  overflow:hidden!important;
  background:#101128!important;
}
body:not(.tamaGameEmbedded) #game,
body.gameExpandedFromTama #game{
...
body.petbotOpen.tamaGameEmbedded .instructions,
body.petbotOpen.tamaGameEmbedded .builder,
body.petbotOpen.tamaGameEmbedded .petPanel,
body.petbotOpen.tamaGameEmbedded .touch,
body.petbotOpen.tamaGameEmbedded .helpBtn{display:none!important;}



/* STAGE 27: quest input + on-screen text + smoother full-screen walking */
.tamaGameCaption{
  display:none;position:fixed;left:var(--tama-lcd-left,50vw);top:calc(var(--tama-lcd-top,50dvh) + 8px);width:var(--tama-lcd-w,360px);min-height:26px;transform:none;z-index:2147483600;pointer-events:none;text-align:center;padding:5px 10px;border-radius:999px;background:rgba(255,247,189,.94);color:#21142d;font-weight:1000;font-size:clamp(11px,1.45vw,17px);line-height:1.12;box-shadow:0 8px 18px rgba(0,0,0,.22), inset 0 0 0 2px rgba(255,255,255,.45);text-shadow:none;
}
body.petbotOpen.tamaGameEmbedded .tamaGameCaption.show{display:block!important;}
body.gameExpandedFromTama .tamaGameCaption.show{display:block!important;left:50vw!important;top:auto!important;bottom:calc(env(safe-area-inset-bottom) + 22px)!important;width:min(760px,86vw)!important;transform:translateX(-50%)!important;}
body.gameExpandedFromTama #buildingObjects,body.gameExpandedFromTama .building,body.gameExpandedFromTama .interactiveStop,body.gameExpandedFromTama .lessonBlock,body:not(.petbotOpen) #buildingObjects,body:not(.petbotOpen) .building{pointer-events:auto!important;}
body.gameMoving .building img,body.gameMoving .obj.hasImage img{filter:drop-shadow(0 10px 12px rgba(0,0,0,.24))!important;}
body.gameMoving .building,body.gameMoving .obj,body.gameMoving .character img{transition:none!important;}
body.gameExpandedFromTama #game{will-change:transform!important;contain:layout paint style!important;backface-visibility:hidden!important;}



/* STAGE 28: quest building selection, visible text, and lighter fullscreen walking. */
body.gameExpandedFromTama #toast{display:none!important;}
body.gameExpandedFromTama .tamaGameCaption.show{
  display:block!important;
  position:fixed!important;
  left:50vw!important;
  right:auto!important;
  ...
  border-radius:16px!important;
  background:rgba(255,247,189,.96)!important;
  color:#21142d!important;
  font-size:clamp(14px,1.25vw,20px)!important;
  line-height:1.15!important;
}
body.petbotOpen.tamaGameEmbedded .tamaGameCaption.show{
  display:block!important;
  position:fixed!important;
  left:calc(var(--tama-lcd-left,50vw) + 8px)!important;
  top:calc(var(--tama-lcd-top,50dvh) + var(--tama-lcd-h,180px) - 40px)!important;
  width:calc(var(--tama-lcd-w,360px) - 16px)!important;
  transform:none!important;
  z-index:2147483646!important;
...
body.petbotOpen.tamaGameEmbedded .character{
...
<!-- EDIT SECTION 07: BODY / VISIBLE PAGE STARTS HERE -->
<body>
<!-- EDIT SECTION 08: GAME LAYERS, HUD, BUILDER, PET CARE AND SCREENS -->
<div id="game">
  <div class="layer sky editableTarget" id="skyLayer" data-layer="sky"></div><div class="layer hills editableTarget" id="hillLayer" data-layer="hills"></div><div id="buildingObjects"></div><div class="underFloor"></div><div class="layer groundDecor editableTarget" id="groundLayer" data-layer="ground"></div><div class="platform editableTarget" id="platform" data-layer="platform"></div>
  <header class="hud"><div class="title"><strong>🚑 Ax + Pura Waaambulance Quests</strong><span id="questName">Main menu: choose a quest.</span></div><div class="stats"><button class="pill hudMenu" id="menuBtn">Menu</button><span class="pill gold">Score: <b id="score">0</b></span><span class="pill pink">Lives: <b id="lives">3</b></span><span class="pill blue">Level: <b id="level">1</b></span><span class="pill">Distance: <b id="distance">0</b>m</span><button class="pill" id="swapBtn">Swap</button><button class="pill" id="pauseBtn">Pause</button><button class="pill purple" id="petBtn">Tamagotchi</button><button class="pill purple" id="buildBtn">Build/Edit 🛠️</button><button class="pill" id="modeBtn">Night 🌙</button></div></header>
  <aside class="instructions hidden" id="instructions"><h2>Quest help</h2><p><b>Move:</b> A/D or arrow keys. <b>Jump:</b> Space, W or ↑. <b>Enter:</b> stand near a glowing building and press Enter. Menu opens Solo/Team, Tamagotchi and Admin Builder.</p></aside>
  <!-- EDIT SECTION 09: ADMIN BUILDER PANEL - sliders, sprite paths, save/load controls -->
  <aside class="builder hidden" id="builder">
    <div class="panelHead"><div><h2>🛠️ Admin Builder</h2><p>Simple editing: pick what you want, drag it on the game, then save. Editor pauses play; Close resumes walking.</p></div><button class="miniClose" id="closeBuilderTop">Close</button></div>
    <div class="builderHelp"><b>How to edit:</b> choose Environment for sky/ground/floor, Objects for buildings/items, Characters for who is playing, Sprites for image paths, then Save. Dragging snaps to an 8px grid so buildings line up more cleanly.</div>
    <div class="quickTabs"><button data-tab="layout" class="active">Environment / Objects</button><button data-tab="chars">Characters</button><button data-tab="sprites">Sprites</button><button data-tab="save">Save / Load</button></div>
    <div id="tab-layout" class="tabBody active">
      <p class="tabHint">Use the big buttons first. The selected item gets a blue dashed outline.</p>
      <div class="editPickGrid"><div class="editorGroupHeader">Environment</div><button class="targetPick" data-select-target="sky">Sky</button><button class="targetPick" data-select-target="hills">Hills background</button><button class="targetPick" data-select-target="ground">Ground artwork</button><button class="targetPick" data-select-target="platform">Invisible floor line</button><button class="targetPick" data-select-target="layers">Layer levels</button><div class="editorGroupHeader">Game Objects</div><button class="targetPick active" data-select-target="player">Main character</button><button class="targetPick" data-select-target="buddy">Side character</button><button class="targetPick" data-select-target="places">Places / buildings</button><button class="targetPick" data-select-target="objects">Pickups / hazards</button></div>
      <label>Selected part</label><select id="editTarget"><option value="player">Main character</option><option value="buddy">Side character</option><option value="places">Places / buildings</option><option value="layers">Layer levels</option><option value="hills">Hills layer</option><option value="ground">Ground artwork</option><option value="platform">Invisible floor line</option><option value="sky">Sky layer</option><option value="objects">Pickups / hazards</option></select>
      <div class="editSection section-places"><h3>Places / buildings</h3><p class="tabHint">Pick the place you want, then click/drag anywhere on the game canvas to place it. Left/right changes where it appears in the route. Up/down changes the feet position.</p><div class="placeEditorGrid"><label>Building<select id="placeSelect"><option value="welcomeBoard">Welcome Board</option><option value="petShop">Pet Shop</option><option value="habitat">Habitat</option><option value="hookWaaambulance">Hook-a-Waaambulance</option><option value="questStarter">Quest Starter</option></select></label><label>Quick action<select id="placeQuick"><option value="">Choose...</option><option value="bringHere">Bring selected building here</option><option value="groundSnap">Snap feet to ground</option></select></label></div><label class="calRow">Level position <input type="range" id="calPlaceAt" min="-3000" max="7000" value="-1719"><span id="calPlaceAtVal">-1719</span></label><label class="calRow">Width <input type="range" id="calPlaceW" min="80" max="900" value="250"><span id="calPlaceWVal">168px</span></label><label class="calRow">Height <input type="range" id="calPlaceH" min="50" max="700" value="200"><span id="calPlaceHVal">200px</span></label><label class="calRow">Feet up/down <input type="range" id="calPlaceY" min="-180" max="180" value="-25"><span id="calPlaceYVal">-25px</span></label><label class="calRow">Building layer <input type="range" id="calPlaceZ" min="0" max="80" value="48"><span id="calPlaceZVal">48</span></label></div>
      <div class="editSection section-player"><h3>Main character</h3><label class="calRow">X position <input type="range" id="calPlayerX" min="0" max="800" value="150"><span id="calPlayerXVal">150px</span></label><label class="calRow">Auto character size <input type="range" id="calPlayerW" min="110" max="450" value="170"><span id="calPlayerWVal">170px</span></label><label class="calRow">Feet up/down <input type="range" id="calDrop" min="-45" max="110" value="1"><span id="calDropVal">1px</span></label></div>
      <div class="editSection section-buddy"><h3>Side character</h3><label class="calRow">X position <input type="range" id="calBuddyX" min="0" max="900" value="265"><span id="calBuddyXVal">265px</span></label><label class="calRow">Auto character size <input type="range" id="calBuddyW" min="70" max="300" value="115"><span id="calBuddyWVal">115px</span></label><label class="calRow">Feet up/down <input type="range" id="calBuddyDrop" min="-45" max="110" value="13"><span id="calBuddyDropVal">13px</span></label></div>
      <div class="editSection section-layers"><h3>World layers</h3><div class="layerStackHint">Layer order: bigger z number = closer to the front. Keep Sky lowest, then Hills, Ground, Buildings, Items, Characters.</div><label class="calRow">Floor line <input type="range" id="calFloor" min="70" max="240" value="70"><span id="calFloorVal">70px</span></label><label class="calRow">Invisible floor height <input type="range" id="calPlatform" min="30" max="110" value="30"><span id="calPlatformVal">30px</span></label><label class="calRow">Hill height <input type="range" id="calHillH" min="80" max="1000" value="445"><span id="calHillHVal">445px</span></label><label class="calRow">Hill bottom <input type="range" id="calHillB" min="-250" max="600" value="82"><span id="calHillBVal">82px</span></label><label class="calRow">Ground height <input type="range" id="calGroundH" min="40" max="1000" value="757"><span id="calGroundHVal">757px</span></label><label class="calRow">Ground bottom <input type="range" id="calGroundB" min="-350" max="350" value="-120"><span id="calGroundBVal">-120px</span></label><h3>Layer levels / front-back order</h3><label class="calRow">Sky z <input type="range" id="calZSky" min="0" max="80" value="0"><span id="calZSkyVal">0</span></label><label class="calRow">Hills z <input type="range" id="calZHills" min="0" max="80" value="1"><span id="calZHillsVal">1</span></label><label class="calRow">Ground z <input type="range" id="calZGround" min="0" max="80" value="39"><span id="calZGroundVal">39</span></label><label class="calRow">Invisible floor z <input type="range" id="calZPlatform" min="0" max="80" value="0"><span id="calZPlatformVal">0</span></label><label class="calRow">Side char z <input type="range" id="calZBuddy" min="0" max="80" value="46"><span id="calZBuddyVal">46</span></label><label class="calRow">Main char z <input type="range" id="calZPlayer" min="0" max="80" value="58"><span id="calZPlayerVal">58</span></label><label class="calRow">Buildings z <input type="range" id="calZBuildings" min="0" max="80" value="25"><span id="calZBuildingsVal">25</span></label><label class="calRow">Items z <input type="range" id="calZObjects" min="0" max="80" value="50"><span id="calZObjectsVal">50</span></label></div>
      <div class="editSection section-objects"><h3>Pickups and hazards</h3><label class="calRow">Token height <input type="range" id="calTokenLift" min="20" max="520" value="126"><span id="calTokenLiftVal">126px</span></label><label class="calRow">Hazard height <input type="range" id="calHazardLift" min="0" max="300" value="58"><span id="calHazardLiftVal">58px</span></label><label class="calRow" style="display:none">Layer front/back <input type="range" id="calZ" min="0" max="70" value="30"><span id="calZVal">30</span></label></div>
    </div>
    <div id="tab-chars" class="tabBody"><p class="tabHint">Choose the visible characters and fix their facing without touching code.</p><label>Main character</label><select id="mainSelect"><option value="ax">Ax</option><option value="pura">Pura</option><option value="owl">Owl</option><option value="unicorn">Unicorn</option><option value="pet">Selected Pet</option></select><label>Main facing</label><select id="mainFlipSelect"><option value="1">Normal</option><option value="-1">Flip left/right</option></select><label>Side character / team follower</label><select id="buddySelect"><option value="none">No side character</option><option value="pura">Pura</option><option value="ax">Ax</option><option value="owl">Owl</option><option value="unicorn">Unicorn</option><option value="pet">Selected Pet</option></select><label>Side character facing</label><select id="buddyFlipSelect"><option value="1">Normal</option><option value="-1">Flip left/right</option></select><p class="tabHint">Tip: if a character looks like it is moonwalking, flip that character here and press Apply characters.</p><div class="calButtons"><button class="good" id="applyCharsBtn">Apply characters</button><button class="warn" id="soloBtn">Make Solo Quest</button><button class="warn" id="teamBtn">Make Team Quest</button></div></div>
    <div id="tab-sprites" class="tabBody"><p class="tabHint">Sprite paths: use {n} for 001, 002, 003 etc. Missing sprites now show a yellow warning instead of silently disappearing.</p><h3>Ax</h3><label>Ax idle URL pattern</label><input id="axIdleUrl"><label>Ax walk URL pattern</label><input id="axWalkUrl"><label>Ax jump URL pattern</label><input id="axJumpUrl"><h3>Pura</h3><label>Pura idle URL pattern</label><input id="puraIdleUrl"><label>Pura walk URL pattern</label><input id="puraWalkUrl"><label>Pura jump URL pattern</label><input id="puraJumpUrl"><h3>Owl</h3><label>Owl idle URL pattern</label><input id="owlIdleUrl"><label>Owl walk URL pattern</label><input id="owlWalkUrl"><label>Owl jump URL pattern</label><input id="owlJumpUrl"><h3>Unicorn</h3><label>Unicorn idle URL pattern</label><input id="unicornIdleUrl"><label>Unicorn walk URL pattern</label><input id="unicornWalkUrl"><label>Unicorn jump URL pattern</label><input id="unicornJumpUrl"><div class="calButtons"><button id="applySpriteUrlsBtn" class="good">Apply sprite URLs</button><button id="resetSpriteUrlsBtn" class="warn">Reset sprite URLs</button></div><div class="statusBox" id="spriteStatus">Sprite URL patterns ready.</div></div>
    <div id="tab-save" class="tabBody"><p class="tabHint">Save keeps building positions and building sizes. It saves a browser backup every time, and shared cloud save when your admin key is entered. Sprite paths and frame counts still come from EDIT SECTION 17/18 in index.html, not the cloud save.</p><label>Admin key</label><input id="adminKey" type="password" placeholder="CTW_GAME_ADMIN_KEY"><label>Level name</label><input id="levelName" value="level_main"><div class="calButtons"><button id="saveCloudBtn" class="good">Save shared level</button><button id="loadCloudBtn">Load shared level</button><button id="resetSettingsBtn" class="bad">Reset safe layout</button><button id="closeBuilderBtn">Close editor</button></div><textarea id="settingsBox" readonly></textarea><div class="statusBox" id="adminStatus">Ready. Drag Places / buildings, then press Save shared level. A browser backup is saved too.</div></div>
  </aside>
  <!-- EDIT SECTION 10: PETBOT TAMAGOTCHI PANEL - loads owned pets and Sheet 51 images -->
  <aside class="petPanel hidden" id="petPanel">
    <div class="tamaShell">
      <div class="tamaTop"><div><h2>🐾 PetBot Tamagotchi</h2><p>Your owned pets come from PetBot. Pictures come from Sheet 51.</p></div><button class="miniClose" id="closePetBtn">Close</button></div>
      <div class="petFormGrid"><label>User<select id="userSelect"><option value="1214671839205396481">Ryan</option><option value="1407957358683885670">Ashely</option></select></label><label>Or paste Discord user ID<input id="userIdInput" placeholder="optional Discord user ID"></label><label style="grid-column:1/-1">Owned pet<select id="petSelect"></select></label></div>
      <div class="tamaScreen" id="tamaScreen">
        <div class="petPreview empty" id="petPreview">No pet loaded yet</div>
        <div class="petInfo"><h3 id="petTitle">No pet selected</h3><p id="petSub">Press Reload live.</p><div class="petBadges"><span class="petBadge" id="petCodeBadge">code —</span><span class="petBadge green" id="petLevelBadge">level —</span><span class="petBadge blue" id="petImageBadge">Sheet 51 image</span></div><div class="petStats"><label>Hunger</label><div class="bar"><span id="hungerBar"></span></div><label>Happiness</label><div class="bar"><span id="happyBar"></span></div><label>Energy</label><div class="bar"><span id="energyBar"></span></div></div></div>
      </div>
      <div class="petSizeControls"><label>Pet game size<input id="petSizeRange" type="range" min="45" max="240" value="115"><span id="petSizeVal">115px</span></label><label>Pet facing<select id="petFlipSelect"><option value="1">Normal</option><option value="-1">Flip left/right</option></select></label></div><div class="tamaControls"><button class="warn" id="reloadPetsBtn">Reload live</button><button class="good" id="usePetBtn">Use pet as player</button><button id="feedVisualBtn">🍔 Feed visual</button><button id="playVisualBtn">🧸 Play visual</button><button id="restVisualBtn">💤 Rest visual</button><button id="healVisualBtn">💊 Heal visual</button></div>
      <div class="statusBox petStatusLine" id="petStatus">PetBot ready.</div>
    </div>
  </aside>

  <!-- PETBOT APP OVERLAY: shows /petbot_test.html inside this index page -->
  <div class="petbotDock" id="petbotDock" aria-label="PetBot Tamagotchi dock">
    <div class="petbotDockPreview" aria-hidden="true"><div class="petbotDockMiniLcd"><div class="miniTitle" id="dockMiniTitle">PetBot</div><div class="miniPet" id="dockMiniPet"><span>♡</span></div><div class="miniBars" id="dockMiniBars"><i></i><i></i><i></i></div></div><img src="petbot_device.png?v=20260628-dock-screen-05" alt=""></div>
    <iframe class="petbotDockFrame" id="petbotDockFrame" title="Waaambulance PetBot Tamagotchi" src="petbot_test.html?v=127&embed=1&launch=1&choose=1" allow="fullscreen" allowfullscreen></iframe>
    <div class="petbotDockBar">
      <button class="petbotDockBtn main" id="petbotExpandBtn" type="button">Small</button>
      <button class="petbotDockBtn" id="petbotHideBtn" type="button">Hide</button>
    </div>
  </div>
  <button class="petbotBackBtn" id="petbotBackBtn" type="button" aria-label="Back to game">← Back to game</button>

  <!-- EDIT SECTION 11: LIVE GAME OBJECTS - characters, world objects, toast, drag hint, touch controls -->
  <div id="teamShadow2"></div><div id="buddyShadow"></div><div id="shadow"></div><div id="teamShadow3"></div><div id="teamMate2" class="character teamMate editableTarget" data-layer="team2"><img id="teamSprite2" alt="team character 3"></div><div id="buddy" class="character editableTarget" data-layer="buddy"><img id="buddySprite" alt="side character"></div><div id="player" class="character editableTarget" data-layer="player"><img id="sprite" alt="main character"></div><div id="teamMate3" class="character teamMate editableTarget" data-layer="team3"><img id="teamSprite3" alt="team character 4"></div><div id="worldObjects"></div><div class="toast" id="toast">Waaambulance activated</div><div class="tamaGameCaption" id="tamaGameCaption" aria-live="polite"></div><div class="axFrameDebug" id="axFrameDebug">Ax frame: —</div><div class="dragHint" id="dragHint">drag selected layer</div>
  <div class="touch"><button id="touchLeft">←</button><button id="touchJump">↑</button><button id="touchRight">→</button><button id="touchJump2" class="wide">JUMP / WEE-WOO</button></div><button class="pill helpBtn" id="helpBtn">?</button>
  <!-- EDIT SECTION 12: START MENU / LEVEL COMPLETE / GAME OVER SCREENS -->
  <div class="overlay hidden" id="startScreen"><div class="modal menuModal"><h1>🚑 Waaambulance Quest Dash</h1><p>Choose Solo or build your own team. Team can be 1, 2, 3 or 4 characters — it no longer forces all four.</p><div class="soloPickerBox"><label>Solo character</label><select id="soloCharacterSelect"><option value="ax">Ax</option><option value="pura">Pura</option><option value="owl">Owl</option><option value="unicorn">Unicorn</option><option value="pet">Selected Pet</option></select></div><div class="teamPickerBox"><p><b>Team characters</b> — pick up to 4. Leave a slot as None if you do not want that many.</p><div class="teamPickerGrid"><label>1 Main<select id="teamSlot1Select"><option value="ax">Ax</option><option value="pura">Pura</option><option value="owl">Owl</option><option value="unicorn">Unicorn</option><option value="pet">Selected Pet</option></select></label><label>2 Follower<select id="teamSlot2Select"><option value="none">None</option><option value="ax">Ax</option><option value="pura">Pura</option><option value="owl">Owl</option><option value="unicorn">Unicorn</option><option value="pet">Selected Pet</option></select></label><label>3 Follower<select id="teamSlot3Select"><option value="none">None</option><option value="ax">Ax</option><option value="pura">Pura</option><option value="owl">Owl</option><option value="unicorn">Unicorn</option><option value="pet">Selected Pet</option></select></label><label>4 Follower<select id="teamSlot4Select"><option value="none">None</option><option value="ax">Ax</option><option value="pura">Pura</option><option value="owl">Owl</option><option value="unicorn">Unicorn</option><option value="pet">Selected Pet</option></select></label></div></div><div class="choiceRow"><button id="resumeBtn" class="secondary">Resume game</button><button id="startSoloBtn">Solo Quest</button><button id="startTeamBtn" class="good">Start Team Quest</button><button id="menuPetBtn" class="secondary">Tamagotchi</button><button id="menuBuildBtn" class="dark">Admin Builder</button></div><p style="font-size:13px;opacity:.85;margin-top:12px">Controls: arrows/space. Menu pauses the game and brings this screen back.</p></div></div>
  <div class="overlay hidden" id="doorScreen"><div class="doorModal"><h1 id="doorTitle">Opening...</h1><img id="doorBigImg" src="https://res.cloudinary.com/dpwlfmhia/image/upload/v1782174047/door_1.png" alt="Opening door"></div></div>
  <div class="overlay hidden" id="shopScreen"><div class="modal"><h1>🐾 Pet Shop</h1><p>This screen is ready for the Discord App upgrade. Later it can read PetBot shop stock, coins and buying from the Worker/API.</p><div class="shopGrid"><div class="shopCard"><strong>🍔 Food</strong><span>PetBot food items will load here.</span></div><div class="shopCard"><strong>🧸 Toys</strong><span>Play and happiness items will load here.</span></div><div class="shopCard"><strong>🏡 Habitat items</strong><span>Decor and habitat items will load here.</span></div><div class="shopCard"><strong>💊 Care extras</strong><span>Healing, rest and training items will load here.</span></div></div><button id="closeShopBtn">Close Pet Shop</button></div></div>
  <div class="overlay hidden" id="welcomeInfoScreen"><div class="modal"><h1>🪧 Welcome Board</h1><p><b>How to play:</b> move with Right/D and Left/A. Jump with Space, W or Arrow Up.</p><p><b>Tutorial route:</b> Controls → Pet Shop → Habitat → Jump Practice → Hook-a-Waaambulance → Quest Starter → Finish.</p><p><b>Buildings:</b> stand near a glowing building, then press Enter or tap/click it. Yellow boards tell you what the next stop does.</p><button id="closeWelcomeBtn">Close Welcome Board</button></div></div>
  <div class="overlay hidden" id="hookScreen"><div class="modal"><h1>🎣 Hook-a-Waaambulance</h1><p>This is now its own place instead of being inside the Pet Shop menu. Later this screen can open the hook-a-duck style prize game.</p><div class="shopGrid"><div class="shopCard"><strong>🎟️ Tickets</strong><span>Show tickets and entry cost here.</span></div><div class="shopCard"><strong>🏆 Prizes</strong><span>Load prize pool from your bot/sheet here.</span></div></div><button id="closeHookBtn">Close Hook-a-Waaambulance</button></div></div>
  <div class="overlay hidden" id="questStartScreen"><div class="modal"><h1>🗺️ Quest Starter</h1><p>Choose which quest to start. This is the building that will become your quest selector.</p><div class="choiceRow"><button data-quest-level="0">Tutorial Town</button><button data-quest-level="1">Wrong Moon</button><button data-quest-level="2">Swamp Monster</button></div><button id="closeQuestStarterBtn">Close Quest Starter</button></div></div>
  <div class="overlay hidden" id="levelScreen"><div class="modal"><h1 id="levelTitle">Level complete</h1><p id="levelText">The quest chaos continues.</p><button id="nextLevelBtn">Next level</button></div></div><div class="overlay hidden" id="endScreen"><div class="modal"><h1 id="endTitle">Patrol over</h1><p id="endText">The Waaambulance is being dramatic.</p><button id="restartBtn">Restart</button></div></div>
</div>
<!-- EDIT SECTION 15: JAVASCRIPT STARTS HERE - GAME LOGIC BELOW -->
<script>
...
function expandEmbeddedTamaGame(){
  ...
  try{if(!document.fullscreenElement&&document.documentElement.requestFullscreen){const r=document.documentElement.requestFullscreen();if(r&&r.catch)r.catch(()=>{});}}catch(e){}
  showToast('Full-screen quest: stand near a glowing building and press Enter, Space, or tap it.');
  requestAnimationFrame(()=>{try{fitGameToScreen();}catch(e){}});
}
...
(function(){
  ...
  function placeToPetbotScreen(place){
    ...
    if(p==='welcomeBoard') return 'petHome';
    return 'petActions';
  }
  function postPetbotOpen(place,label){
    ...
    try{ if(typeof paused!=='undefined') paused=true; }catch(e){}
    const frame=q$('petbotFrame');
    const msg={type:'waa-parent-open-screen',screen,place,label:label||place};
    setTimeout(()=>{try{frame&&frame.contentWindow&&frame.contentWindow.postMessage(msg,location.origin)}catch(e){}},90);
    setTimeout(()=>{try{frame&&frame.contentWindow&&frame.contentWindow.postMessage(msg,location.origin)}catch(e){}},320);
...
(function(){
  ...
  function petbotFrameUrl(){
    ...
    return 'petbot_test.html?v=127&embed=1&launch=1&choose=1';
  }
